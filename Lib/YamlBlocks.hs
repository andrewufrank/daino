...
module Lib.YamlBlocks
  ( module Lib.YamlBlocks
  , Value(..), putStringAtKey
  , getMaybeStringAtKey 
  , readMarkdown2 
  , readMd2meta
  , readYaml2value
  , YamlText(..), yamlFileType
  )
where
...
instance AtKey DocValue  Text where
    ...
    putStringAtKey  k2 txt meta2= DocValue $ putStringAtKey k2 txt (unDocValue meta2)

instance AtKey DocValue Bool  where
    getMaybeStringAtKey meta2 k2 =  getMaybeStringAtKey (unDocValue meta2) k2

    putStringAtKey  k2 b meta2 = DocValue $ putStringAtKey k2 b (unDocValue meta2)
